...
aregs_t     *p_regs = GET_REGS_PTR(unit);
     ...
     || (p_config->chan_mask_groupc & ADC_MASK_SENSORS))
    {
        if ((0 == unit)
         ...
         || (0 != S12AD1.ADDISCR.BYTE)         // disconnect detection assist enabled
...
static void adc_configure_sensors(adc_ch_cfg_t   *p_config)
{
    ...
    if ((p_config->chan_mask & ADC_MASK_TEMP) || (p_config->chan_mask_groupb & ADC_MASK_TEMP) || (p_config->chan_mask_groupc & ADC_MASK_TEMP))
    {
        ...
        else if (p_config->chan_mask_groupb & ADC_MASK_TEMP)
        {
            S12AD1.ADEXICR.BIT.TSSA = 0;
...
static void adc_enable_s12gcadi(uint8_t unit)
{
    ...
    if (0 == unit)
    {
        ...
        {
            IEN(S12ADC0,S12GCADI0) = 1;       // enable in ICU
        }
    }
    ...
    else /* Unit1 */
    {
        ...
        {
            IEN(S12ADC1,S12GCADI1) = 1;       // enable in ICU
        }
    }
}
...
* Return Value : none
******************************************************************************/
#pragma interrupt adc_s12adi0_isr(vect=VECT(S12ADC0,S12ADI0))
...
* Return Value : none
******************************************************************************/
#pragma interrupt adc_s12adi1_isr(vect=VECT(S12ADC1,S12ADI1))
...
* Return Value : none
******************************************************************************/
#pragma interrupt adc_s12gbadi0_isr(vect=VECT(S12ADC0,S12GBADI0))
...
* Return Value : none
******************************************************************************/
#pragma interrupt adc_s12gbadi1_isr(vect=VECT(S12ADC1,S12GBADI1))
...
* Return Value : none
******************************************************************************/
#pragma interrupt adc_s12gcadi0_isr(vect=VECT(S12ADC0,S12GCADI0))
...
* Return Value : none
******************************************************************************/
#pragma interrupt adc_s12gcadi1_isr(vect=VECT(S12ADC1,S12GCADI1))
...
volatile    uint8_t i;
    ...
    if (0 == unit)
    {
        ...
        IEN(S12ADC0,S12GCADI0) = 0;       // disable interrupts in ICU
    ...
    else /* Unit1 */
    {
        ...
        IEN(S12ADC1,S12GCADI1) = 0;       // disable interrupts in ICU
