...
*         : 31.01.2017 1.01_ES   Added callback processing of the following group interrupt sources.
*                                - RIIC1_EEI1
*                                - RIIC1_TEI1
*                                - GLCDC_VPOS
*                                - GLCDC_GR1UF
*                                - GLCDC_GR2UF
*                                - DRW2D_DRW_IRQ
*                                Changed calling condition of callback function in group interrupts.
...
#else                          /* In the case of 512K, 768K or 1Mbyte ROM capacity. */
...

{
    ...
    if ((1 == ICU.GRPBL1.BIT.IS29) && (1 == ICU.GENBL1.BIT.EN29))
    {
        R_BSP_InterruptControl(BSP_INT_SRC_BL1_RIIC1_EEI1, BSP_INT_CMD_CALL_CALLBACK, FIT_NO_PTR);
    }

    if ((1 == ICU.GRPBL1.BIT.IS28) && (1 == ICU.GENBL1.BIT.EN28))
    {
...
* Return Value : None
***********************************************************************************************************************/
#pragma vector=VECT(ICU,GROUPBL2)
__interrupt void group_bl2_handler_isr (void)

{
    if ((1 == ICU.GRPBL2.BIT.IS0) && (1 == ICU.GENBL2.BIT.EN0))
    {
...
__interrupt void group_al1_handler_isr (void)

{
    if ((1 == ICU.GRPAL1.BIT.IS4) && (1 == ICU.GENAL1.BIT.EN4))
    {
