...
　
　Envision Kit用のプロジェクトに立ち戻ってデバッグ開始。
　現象を見た感じ、レジスタアクセスがうまくいってなくてクロック設定が
　おかしくなっているような気がする。
　最適化を切ってみる。確かGCC移植開始時も最適化OFFにしたはず。
　
　ここは直った。最適化周りはvolatileとかevenaccessとかをいじり倒しているから
　何か不整合があっても不思議ではない。あとで調査する。課題★
　
　ただしまだ続きで暴走してる。今度は未定義命令割り込み。
　割り込みベクタ周りのファイルが先祖返りしたか。
　
　2018/07/14-(2)で実施した以下あたりが消え去っていた。思い出しながら再度修正を施す。
　>>port.c でvTickISR()は割り込み関数指定されているがこれを解除。
　
　main()まで到達するようになった。UARTで数文字送ったところで暴走。
　見たことない現象。なんだろう。トレースしてみる。
　未定義割り込みが発生している。
　
　Etherが属するグループ割り込みがおかしいようだ。
　freertos_usr_func.c にはみ出てるグループ割り込みがコメントアウトされてる。
　IAR対応の時にコメントアウトしたような気がしてきた。
　
　例のNoMaY氏のマクロ
　
　
